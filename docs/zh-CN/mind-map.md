...
如果当前环境是 ChatGPT，并且用户要求思维导图图片，或 DOCX/完整复习包需要更直观的图示，应优先使用 GPT 图像模型 2 或 ChatGPT 内置图像生成工具，把已经校验过的 Mermaid、缩进树或层级大纲转换为准确、美观、清晰、适合打印的思维导图图片。

图片生成必须遵守：

1. **先结构，后图像**：先基于上传资料生成 Mermaid、缩进树或层级大纲，再据此生成图片。
2. **准确优先**：节点文字、层级关系、公式/流程/算法/图表线索、必考/易错/题型标签必须与结构化导图一致。
3. **不新增资料外内容**：不得为了美化而加入上传资料或用户指定范围之外的节点。
4. **清晰可读**：字体清楚，主干突出，分支不拥挤，适合打印和考前复习。
5. **美观克制**：可使用柔和配色、圆角节点、清晰连线和轻量图标，但不能牺牲信息准确性。
6. **生成后校验**：如果图片存在文字错误、节点遗漏或层级错乱，应以 Mermaid / 缩进树为可靠来源，并重新生成或补充说明。
...
- [ ] 是否避免了泛泛模板和装饰性节点。
- [ ] 是否配有读图说明。
- [ ] 如果生成图片，图片是否与结构化导图一致、准确、清晰、美观、适合打印。